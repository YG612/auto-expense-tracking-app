...
if (-not $VerifyOnly) {
  ...
  $prepared = [ordered]@{
    schemaVersion = 1
    engine = 'onnx'
    runtimeSha256 = $lock.runtime.sha256
    modelArchiveSha256 = $lock.model.archiveSha256
  } | ConvertTo-Json
  Set-Content -LiteralPath (Join-Path $assetsRoot 'prepared-assets.json') -Value $prepared -Encoding utf8
}

Assert-LockedFile $runtimeTarget $lock.runtime.sizeBytes $lock.runtime.sha256
foreach ($name in $lock.model.requiredFiles) {
  if (-not (Test-Path -LiteralPath (Join-Path $assetsRoot $name) -PathType Leaf)) {
    throw "Missing prepared model file: $name"
  }
}
