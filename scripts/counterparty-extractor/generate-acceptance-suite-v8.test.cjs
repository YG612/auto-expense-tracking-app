...
const { generate } = require('./generate-acceptance-suite-v8.cjs');

test('v8 final blind suite is balanced and span-valid', () => {
  const rows = generate();
  const positives = rows.filter(row => row.counterparty !== null);
  const negatives = rows.filter(row => row.counterparty === null);
  assert.ok(rows.length >= 315);
  assert.ok(positives.length >= 145);
