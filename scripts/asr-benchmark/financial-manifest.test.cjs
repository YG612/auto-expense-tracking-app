...
test('expanded corpus covers required financial and acoustic slices', () => {
  ...
  for (const tag of ['expense', 'income', 'refund', 'transfer', 'reimbursement', 'repayment', 'multi_transaction', 'quantity_unit_price', 'negation']) assert.ok(tags.has(tag), `missing scene tag ${tag}`);
