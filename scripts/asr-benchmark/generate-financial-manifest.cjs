...
const { extractAmountSequenceFen, extractNumberSequence, parseJsonLines } = require('./score-asr-ab.cjs');
...
for (let templateIndex = 0; templateIndex < templates.length; templateIndex += 1) {
  for (let amountIndex = 0; amountIndex < amounts.length; amountIndex += 1) {
    ...
    if (JSON.stringify(extractAmountSequenceFen(spec.text)) !== JSON.stringify(expectedFen)) throw new Error(`${id}: generated amount sequence is inconsistent`);
    if (JSON.stringify(extractNumberSequence(spec.text)) !== JSON.stringify(spec.numbers)) throw new Error(`${id}: generated number sequence is inconsistent`);
    generated.push({
      id,
