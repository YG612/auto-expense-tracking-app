...
const baseCases = parseJsonLines(
  readFileSync(manifestPath, 'utf8'),
  manifestPath,
).filter(entry => Number.parseInt(entry.id.slice(4), 10) <= 22);

const amounts = [
  3, 5, 7, 9, 12, 15, 18, 20, 25, 28, 32, 35, 48, 50, 68, 88, 120, 200,
];
const environments = [
  'clean',
  'cafe_noise',
...
const templates = [
  ...
  amount => ({
    text: `8月10号打车花了${amount}元`,
    amounts: [amount],
    numbers: [8, 10, amount],
    ledger: amount,
    tags: ['expense', 'transport', 'date'],
  }),
  amount => ({
    text: `午饭${amount}元晚饭${amount + 7}元`,
    amounts: [amount, amount + 7],
    numbers: [amount, amount + 7],
    ledger: amount * 2 + 7,
    tags: ['expense', 'multi_transaction'],
  }),
  amount => ({
    text: `商家退款${amount}元到微信`,
    amounts: [amount],
    numbers: [amount],
    ledger: amount,
    tags: ['refund', 'account'],
  ...
  amount => ({
    text: `用银行卡还款${amount}元`,
    amounts: [amount],
    numbers: [amount],
    ledger: amount,
    tags: ['repayment', 'account'],
  }),
  amount => ({
    text: `买药花了${amount}元微信付的`,
    amounts: [amount],
    numbers: [amount],
    ledger: amount,
    tags: ['expense', 'medical', 'account'],
  }),
  amount => ({
    text: `先停顿再说买水果花了${amount}元`,
    amounts: [amount],
    numbers: [amount],
    ledger: amount,
    tags: ['expense', 'pause_mid_utterance'],
  }),
  amount => ({
    text: `本来想花${amount}元但是最后没买`,
    amounts: [amount],
    numbers: [amount],
    ledger: null,
    tags: ['planned', 'negation', 'no_transaction'],
  }),
  amount => ({
    text: `买2张票每张${amount}元`,
    amounts: [amount],
    numbers: [2, amount],
    ledger: amount * 2,
    tags: ['expense', 'quantity_unit_price'],
  }),
];
...
for (
  ...
  for (let amountIndex = 0; amountIndex < amounts.length; amountIndex += 1) {
    ...
    if (
      JSON.stringify(extractAmountSequenceFen(spec.text)) !==
      JSON.stringify(expectedFen)
    ) {
      throw new Error(`${id}: generated amount sequence is inconsistent`);
    }
    if (
      JSON.stringify(extractNumberSequence(spec.text)) !==
      JSON.stringify(spec.numbers)
    ) {
    ...
    generated.push({
      ...
      sceneTags: spec.tags,
      environment:
        environments[(templateIndex + amountIndex) % environments.length],
      accentProfile:
        accents[(templateIndex * 3 + amountIndex) % accents.length],
...
if (allCases.length < 200)
  throw new Error('financial manifest must contain at least 200 cases');
writeFileSync(
  manifestPath,
  `${allCases.map(entry => JSON.stringify(entry)).join('\n')}\n`,
);
