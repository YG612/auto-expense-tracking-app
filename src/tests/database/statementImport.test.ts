...
describe('StatementImportRepository', () => {
  ...
  it('resolves a common merchant legal entity during official bill import', async () => {
    const repositories = createRepositories(database);
    const review = await repositories.statementImport.analyze(
      preview('wx-t3-ride', '南京领行科技股份有限公司'),
    );

    expect(review.rows[0]?.candidate).toMatchObject({
      accountIdHint: 'account-wechat',
      categoryIdHint: 'category-expense-transport',
      subcategoryIdHint: 'category-expense-transport-taxi',
      classificationSource: 'MERCHANT_NAME',
    });
  });

  it('batch reviews and ignores imported pending candidates without touching other drafts', async () => {
    const repositories = createRepositories(database);
    const committed = await repositories.statementImport.commit(
      await repositories.statementImport.analyze(preview()),
      '2026-08-13T13:00:00.000Z',
    );
    const imported = await repositories.transactions.findById(
      committed.transactionIds[0]!,
    );
    const reference = { id: imported!.id, revision: imported!.revision };

    await expect(
      repositories.transactions.reviewImportedPendingBatch(
        [reference],
        {
    ...
    ).resolves.toMatchObject({ confirmedIds: [imported!.id] });
    const reviewed = await repositories.transactions.findById(imported!.id);
    ...
    await expect(
      repositories.transactions.confirmPending(
        { id: reviewed!.id, revision: reviewed!.revision },
        '2026-08-13T13:02:00.000Z',
      ),
    ).resolves.toMatchObject({ status: 'APPLIED' });

    const ignoredBatch = await repositories.statementImport.commit(
      await repositories.statementImport.analyze(preview('wx-import-ignore')),
      '2026-08-13T14:00:00.000Z',
    );
    const ignored = await repositories.transactions.findById(
      ignoredBatch.transactionIds[0]!,
    );
    await expect(
      repositories.transactions.softDeletePendingBatch(
        [{ id: ignored!.id, revision: ignored!.revision }],
        '2026-08-13T14:01:00.000Z',
      ),
    ).resolves.toMatchObject({ confirmedIds: [ignored!.id] });
    await expect(
      repositories.transactions.findById(ignored!.id),
    ).resolves.toBeUndefined();
  });

  it('accepts an income category when reviewing an imported refund', async () => {
    const repositories = createRepositories(database);
    const analyzed = await repositories.statementImport.analyze(
      preview('wx-import-refund'),
    );
    const refundReview = {
      ...analyzed,
    ...
    await expect(
      repositories.transactions.reviewImportedPendingBatch(
        [{ id: imported!.id, revision: imported!.revision }],
