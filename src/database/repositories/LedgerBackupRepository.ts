...
const TABLES: readonly TableSpecification[] = [
  ...
  { name: 'privacy_settings', orderBy: 'id' },
  { name: 'import_mapping_templates', orderBy: 'id' },
  { name: 'recurring_templates', orderBy: 'id' },
  { name: 'transactions', orderBy: 'occurred_at, id' },
  { name: 'product_value_events', orderBy: 'occurred_at, id' },
...
async function insertRows(
  ...
  for (const [index, row] of rows.entries()) {
    ...
    if (canonicalJson(actual) !== canonicalJson(expected)) {
      throw new Error(
        `Backup columns for ${table}[${index}] do not match this app version.`,
      );
    }
  }
...
export class LedgerBackupRepository {
  ...
  async restoreBackupDocument(
    ...
    if (document.schemaVersion !== currentSchemaVersion) {
      throw new Error(
        `Backup schema ${document.schemaVersion} does not match app schema ${currentSchemaVersion}.`,
      );
    }
