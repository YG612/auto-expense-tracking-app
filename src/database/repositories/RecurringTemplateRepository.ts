...
function fromRow(row: TemplateRow): RecurringTemplate {
  return {
    ...
    confirmationPolicy: requiredString(
      row,
      'confirmation_policy',
    ) as RecurringTemplate['confirmationPolicy'],
...
export class RecurringTemplateRepository {
  ...
  async create(template: RecurringTemplate): Promise<void> {
    ...
    if (!['DRAFT', 'AUTO'].includes(template.confirmationPolicy)) {
      throw new Error('周期确认策略无效。');
    }
    ...
    await this.database.transaction(async executor => {
      await categoryAssignment(template.categoryId, executor);
      ...
      await executor.execute(
        `INSERT INTO recurring_templates (
          ...
          monthly_anchor_is_end_of_month, confirmation_policy, enabled,
        ...
        while (occurrence <= canonicalNow && perTemplate < 36) {
          ...
          if (existing.rows[0] === undefined) {
            const transaction: Transaction = {
              id: `recurring-${sha256(sourceReferenceId).slice(0, 32)}`,
              revision: 1,
              type: template.type,
              ...
              reviewReasonCodes: [],
              confirmationStatus:
                template.confirmationPolicy === 'AUTO'
                  ? 'CONFIRMED'
                  : 'PENDING',
              autoConfirmationReason:
                template.confirmationPolicy === 'AUTO'
                  ? `用户已为周期模板“${template.name}”明确开启自动入账`
                  : undefined,
              autoConfirmedAt:
                template.confirmationPolicy === 'AUTO'
                  ? canonicalNow
