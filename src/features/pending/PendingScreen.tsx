...
type BatchModal = 'CATEGORY' | 'ACCOUNT';
type CardModal = { transactionId: string; kind: BatchModal };
  ...
  return (
    <View style={[styles.reviewField, missing && styles.reviewFieldMissing]}>
      ...
      >
        {choices.map(choice => (
          <Pressable
            accessibilityLabel={`${label}：${choice.detail === undefined ? choice.label : `${choice.detail} / ${choice.label}`}${choice.recommendation === 'MOST_LIKELY' ? '，最可能' : ''}`}
            accessibilityRole="button"
          ...
          >
            ...
            >
              {choice.recommendation === 'MOST_LIKELY'
                ? `最可能${choice.detail === undefined ? '' : ` · ${choice.detail}`}`
                : choice.recommendation === 'ALTERNATIVE'
                  ? `备选${choice.detail === undefined ? '' : ` · ${choice.detail}`}`
                  : (choice.detail ?? '常用')}
            </Text>
          </Pressable>
        ))}
...
export function PendingScreen() {
  ...
  const [batchModal, setBatchModal] = useState<BatchModal>();
  ...
  const imported = useMemo(
    () =>
      transactions.filter(
        transaction => transaction.importRecordId !== undefined,
      ),
    [transactions],
  );
  ...
  const applyBatchAssignment = async (kind: BatchModal, id: string) => {
    setBatchModal(undefined);
    if (!beginBusy(`assign-${kind}`)) return;
    try {
      const result = await repositories.transactions.reviewImportedPendingBatch(
        selectedReferences(),
        kind === 'CATEGORY' ? { categoryId: id } : { accountId: id },
        new Date().toISOString(),
      );
      await reloadPending();
      Alert.alert(
        '批量修改完成',
        `已更新 ${result.confirmedIds.length} 笔导入候选。`,
      );
  ...
  const applyCardAssignment = async (
    ...
    if (!beginBusy(`assign-${transaction.id}-${kind}`)) return;
    setError(undefined);
    try {
      const result = await repositories.transactions.reviewImportedPendingBatch(
      ...
      if (!result.confirmedIds.includes(transaction.id)) {
        ...
        { text: '取消', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              Alert.alert(
                '已忽略',
                `${result.confirmedIds.length} 笔记录已移入回收站。`,
              );
            } catch (ignoreError) {
              setError(
                safeErrorMessage(
  ...
  return (
    <SafeAreaView edges={['left', 'right', 'bottom']} style={styles.safeArea}>
      ...
      {imported.length === 0 ? null : (
        <View style={styles.selectionBar}>
          <Pressable
            ...
            onPress={() =>
              setSelectedIds(
                selectedIds.size === imported.length
                  ? new Set()
          ...
          >
            <Text style={styles.selectionButtonText}>
              {selectedIds.size === imported.length ? '取消全选' : '全选导入'}
            </Text>
          </Pressable>
          ...
          {selected.length === 0 ? null : (
            ...
            >
              ...
              <Pressable
                accessibilityRole="button"
                disabled={selectedConfirmable.length === 0}
              ...
              <Pressable
                ...
                style={[styles.batchAction, styles.ignoreAction]}
              >
      ...
      {loading ? (
        ...
        <ScrollView contentContainerStyle={styles.list}>
          {transactions.map(transaction => {
            ...
            const canInlineEdit = transaction.importRecordId !== undefined;
            return (
              <PendingCard
                accountChoices={canInlineEdit ? accountSuggestions.quick : []}
                busy={busyOperation !== undefined}
                categoryChoices={canInlineEdit ? categorySuggestions.quick : []}
                key={transaction.id}
                onChooseAccount={
                  canInlineEdit
                    ? id => applyCardAssignment(transaction, 'ACCOUNT', id)
                    : undefined
                }
                onChooseCategory={
                  canInlineEdit && categorySuggestions.all.length > 0
                ...
                onToggleSelected={
                  transaction.importRecordId === undefined
                    ? undefined
                    : () =>
      ...
      <SelectionModal
        onChange={ids => {
          const id = ids[0];
          if (id !== undefined && batchModal !== undefined) {
            applyBatchAssignment(batchModal, id).catch(() => undefined);
          }
        }}
        onClose={() => setBatchModal(undefined)}
        options={batchModal === 'CATEGORY' ? categoryOptions : accountOptions}
        selectedIds={[]}
        title={batchModal === 'CATEGORY' ? '批量设置分类' : '批量设置账户'}
        visible={batchModal !== undefined}
      />
      <SelectionModal
        onChange={ids => {
          const id = ids[0];
          ...
          if (
            ...
            applyCardAssignment(transaction, modal.kind, id).catch(
              () => undefined,
            );
          }
        }}
        ...
        options={(() => {
          ...
          return cardModal?.kind === 'CATEGORY'
        ...
        selectedIds={(() => {
          ...
          if (transaction === undefined) return [];
          const id =
            cardModal?.kind === 'CATEGORY'
              ? (transaction.subcategoryId ?? transaction.categoryId)
              : transaction.accountId;
          return id === undefined ? [] : [id];
        })()}
        title={cardModal?.kind === 'CATEGORY' ? '选择分类' : '选择账户'}
