...
export type RankedPendingOptions = {
  all: SelectionOption[];
...
export function pendingCategoryOptions(
  ...
  const orderedIds = uniqueIds([selectedId, recognizedId, ...alternativeIds]);
  const recommendationIds = uniqueIds([
    recognizedId ?? selectedId,
    ...alternativeIds,
  ]);
...
export function pendingAccountOptions(
  ...
  const orderedIds = uniqueIds([transaction.accountId, sourceId]);
  const recommendationIds = uniqueIds([sourceId ?? transaction.accountId]);
  const all = moveRecommendedFirst(
