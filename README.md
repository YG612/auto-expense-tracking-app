...
阶段 8（Android 通知自动记账）已形成候选实现：用户显式开启并授予通知使用权后，只筛选微信/支付宝支付结果；事件先原子写入不参与备份的私有事件箱，再由系统 `JobScheduler` 启动 Headless JS 自动生成待确认账，队列未清空时退避重试，后台仍受限时在 App 回前台补导。Android 13+ 的待确认系统提醒使用独立、可拒绝的通知权限。微信/支付宝 CSV 可通过系统文件选择器导入，并支持 UTF-8 与 GBK/GB18030 文本。详见 `docs/AUTO_BOOKKEEPING_RESEARCH_2026-08-15.md`；发布前仍必须完成其中的授权真实通知样本与 OEM 真机矩阵。
