...
“轻记 AI”是一款面向 Android 与 iOS 的本地优先个人智能记账 App。本仓库已包含账本、智能记账、个性化学习、微信/支付宝官方账单导入，以及 Android 支付通知辅助记账的候选实现。支付通知只生成待确认交易，不等同于平台官方 API 的完整账单同步。功能完成不等于第一版已达到生产发布条件；隐私、数据生命周期、生产签名、iOS 构建和第一版缺失项以 `docs/` 中的门禁为准。
...
设置页已接入本地个性化开关、分类规则管理、账单导入和 Android 支付通知辅助入口；跨设备同步仍不在当前范围内。
...
文字解析不会自动创建项目或标签，也没有实现联网 AI 和重复交易合并。支付通知与账单文件通过独立 importer 进入相同的分类、重复检查和待确认边界；只有用户确认并实际纠正后，才会按下述严格条件形成本地商户规则。
...
- 阶段 8：Android 微信/支付宝通知监听、最小 DTO、本地解析、商户预分类、重复检查与待确认

阶段 8 是尽力而为的辅助入口：用户必须在 Android 系统设置显式授予通知使用权，系统或支付 App 未投递、已撤回或模板无法可靠解析的通知不会生成候选。原生队列只驻留内存，只允许微信和支付宝包名，不保存通知原文；交易候选必须经过用户审核。完整历史仍应以微信/支付宝官方账单文件导入为准。
